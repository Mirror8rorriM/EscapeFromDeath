...
void ResourceManager::loadTexts() {
    texts_.clear();
    const auto textsPath = path("texts.txt");
    if (!std::filesystem::exists(textsPath)) {
        return;
    }

    std::ifstream in(textsPath, std::ios::binary);
    if (!in) throw std::runtime_error("Cannot open texts.txt");
    ...
    auto flush = [&]() {
        while (!currentValue.empty() && (currentValue.back() == '\n' || currentValue.back() == '\r')) {
            currentValue.pop_back();
        }
        if (!currentKey.empty()) {
            texts_[currentKey] = currentValue;
        }
        currentValue.clear();
    };
    ...
    while (std::getline(lines, line)) {
        ...
        if (trimmed.size() >= 3 && trimmed.front() == '[' && trimmed.back() == ']') {
            flush();
            currentKey = trimmed.substr(1, trimmed.size() - 2);
            continue;
        }
...
const std::map<std::string, std::string>& ResourceManager::texts() const {
    return texts_;
}

const std::string& ResourceManager::text(const std::string& key) const {
    auto it = texts_.find(key);
    if (it == texts_.end()) throw std::runtime_error("Unknown text key: " + key);
    return it->second;
}
